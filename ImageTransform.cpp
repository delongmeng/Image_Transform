...
PNG illinify(PNG image) {
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      HSLAPixel & pixel = image.getPixel(x, y);
      double d_11 = min(abs(pixel.h - 11), abs(360 + 11 - pixel.h));
      double d_216 = min(abs(pixel.h - 216), abs(360 + 216 - pixel.h));
      if (d_11 < d_216)
        pixel.h = 11;
      else
        pixel.h = 216;
